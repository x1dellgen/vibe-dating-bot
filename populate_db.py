...
async def populate_database():
    # Список анкет, адаптированный под наши новые поля (Scope и Avatar)
    test_profiles = [
    ...
    async with aiosqlite.connect(DB_NAME) as conn:
        # Очищаем старые взаимодействия для чистоты тестов
        await conn.execute("DELETE FROM interactions")
        
        # Загружаем анкеты со всеми 11 полями
        ...
        await conn.commit()
    print("🎉 База данных успешно заселена тестовыми анкетками! Таблица лайков очищена.")
